...
public class image_util_download {
    ...
    public static File SaveImage(Context context, String imageUrl, String fileName) {
        ...
        File file = null;
        try {
            File folder = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_PICTURES), "ChatEast");
            if (!folder.exists()) {
                folder.mkdirs();
            }
            ...
            rotated_bitmap.compress(Bitmap.CompressFormat.JPEG, 100, output_stream); // Increased quality
            output_stream.flush();
            output_stream.close();
